...
def cnv_rna(
    ...
    interim = []
    for gene, keys in sorted(gene_pairs.items()):
        if len(keys) < 4:
            continue
        cnv_vals = [cnv[k].get("cnv_value", 0) if cnv[k].get("cnv_value") is not None else 0 for k in keys]
        rna_vals = [rna[k].get("value", 0) if rna[k].get("value") is not None else 0 for k in keys]
        r, p = sp_stats.pearsonr(cnv_vals, rna_vals)
        case_ids = tuple(sorted({k[0] for k in keys}))
        sample_ids = tuple(sorted({k[1] for k in keys}))
        interim.append((gene, keys, r, p, case_ids, sample_ids))

    if not interim:
        return {"result": NonEstimableResult(reason="insufficient_common_pairs")}

    p_values = [x[3] for x in interim]
    q_values = benjamini_hochberg(p_values)

    findings = []
    for (gene, keys, r, p, case_ids, sample_ids), q in zip(interim, q_values, strict=False):
...
def mutation_rna(
    ...
    for m in mutation_rows:
        gid = m.get("gene_id")
        cid = m.get("case_id")
        if gid and cid and (not variant_classes or m.get("variant_classification") in variant_classes):
    ...
    for gid, mutated_cases in sorted(mutations_by_gene.items()):
        ...
        mut_ids = []
        nonmut_ids = []
        for cid in eligible_sample_ids:
            vals = rna_by_case.get((cid, gid), [])
            if cid in mutated_cases and vals:
                mut_values.extend(vals)
                mut_ids.append(cid)
            elif vals:
                nonmut_values.extend(vals)
                nonmut_ids.append(cid)
        if len(mut_values) < 2 or len(nonmut_values) < 2:
            continue
        t_stat, p_val = sp_stats.ttest_ind(mut_values, nonmut_values, equal_var=False)
        findings.append(AssociationResult(
            test="welch_t",
            effect=float(t_stat),
            confidence_interval=(0.0, 0.0),
            p=float(p_val),
            group_ids={"mutated": tuple(mut_ids), "nonmutated": tuple(nonmut_ids)},
            group_n={"mutated": len(mut_values), "nonmutated": len(nonmut_values)},
        ))
    return {"findings": [f.model_dump(mode="json") for f in findings]}