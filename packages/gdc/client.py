...
class GDCClient:
    ...
    async def _json(self, path: str, params: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if int(response.headers.get("content-length", "0")) > 100_000_000:
            raise GDCResponseError("GDC metadata response exceeds limit")
        try:
            value = response.json()
    ...
    async def paginate(
        self, path: str, *, params: Mapping[str, Any] | None = None, page_size: int | None = None
    ) -> AsyncIterator[dict[str, Any]]:
        """Handle GDC's `pagination` metadata; never infer completion from a magic size."""
        offset, size = 0, page_size or self.page_size
        while True:
            query = dict(params or {}) | {"from": offset, "size": size, "format": "JSON"}
    ...
    async def get_projects(self, *, size: int = 100) -> dict[str, Any]:
        hits = await self.collect(
            "/projects", params={"fields": "project_id,name,primary_site"}, page_size=size
        )
        return {"data": {"hits": hits, "pagination": {"total": len(hits)}}}

    async def get_open_files(self, project_id: str, *, size: int = 500) -> dict[str, Any]:
        fields = (
            "file_id,file_name,file_size,md5sum,access,data_category,data_type,data_format,"
            "experimental_strategy,analysis.workflow_type,cases.case_id,cases.submitter_id,"
            "cases.project.project_id,cases.samples.sample_id,cases.samples.submitter_id,"
            "cases.samples.sample_type,cases.samples.tumor_descriptor,cases.samples.tissue_type,"
            "cases.samples.portions.analytes.aliquots.aliquot_id"
        )
        hits = await self.collect(
            "/files",
            params={"filters": compact_json(open_project_files(project_id)), "fields": fields},
            page_size=size,
        )
        return {"data": {"hits": hits, "pagination": {"total": len(hits)}}}
